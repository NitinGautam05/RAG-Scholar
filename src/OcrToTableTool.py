...
class OcrTableExtractor:
    ...
    def process_all_tables(self):
        ...
        # Initialize and run TableExtractor to get table images
        table_extractor = TableExtractor()
        table_extractor.run()
        
        all_table_images = [
        os.path.join(table_extractor.table_output_dir, f)
        for f in os.listdir(table_extractor.table_output_dir)
        if f.endswith(".png")
    ]
        ...
        for image_path in all_table_images:
            ...
            if not os.path.exists(json_path):  # Only process if JSON doesn't exist
                ...
                print(f"Skipping {image_path}; JSON already exists at {json_path}")
